...
import AddSale from '../compos/addSale';
import { useDeleteCdealMutation, useFindAllCDealsQuery } from '../../redux/API/clientDeals.api';
import { ClientDealFromDB } from '../../types/entities/clientDeal';
const Sales = () => {

  const { isError, error, isLoading, isSuccess, data } = useFindAllCDealsQuery(undefined);
  const [target, setTarget] = useState<ClientDealFromDB>()
  const [deleteOne] = useDeleteCdealMutation()

  if (isError) console.log(error);

  const actions = {
    edit: (data: ClientDealFromDB) => {
      setTarget(data)
      window.addSale.showModal()
    },
    delete: async (data: ClientDealFromDB) => {
      try {
  ...
  return (
    <div className='mx-auto' >
      <button onClick={() => window.addSale.showModal()}>
          ...
          `}
              ...
              <div className='grid absolute inset-0 overflow-auto scrollbar'>
                ...
                {isLoading ? <span className="place-self-center loading loading-spinner loading-lg"></span> :
                  isSuccess && data &&
                  data.map((sale, i) => {
                    const { amount, price, product_name, _id } = sale
                    return (
                      <div>
                        <tr key={_id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 ">
                          ...
                          <td className="px-6 py-4">
                            <button className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                              onClick={() => actions.edit(sale)}
                            >Edit</button>
                          </td>
                          <td className="px-6 py-4">
                            <button className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                              onClick={() => actions.delete(sale)}
                            >Delete</button>
                          </td>
                        </tr>
                      </div>
